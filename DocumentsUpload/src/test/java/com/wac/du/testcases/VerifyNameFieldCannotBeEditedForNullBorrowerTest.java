...
public class VerifyNameFieldCannotBeEditedForNullBorrowerTest extends TestBase {
...
// Test Case: Verify that Name field cannot be edited for Null Borrower
// Scenario: User goes to Documents Upload URL, enters Null Borrower Application ID, verifies applicant Name information,
//			 verifies that Name field cannot be edited
// Expected Results: Name field cannot be edited

	
	  @Test
	  public void verifyNameNotEditableForNullBorrower(ITestContext ctx) {
		  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
			
		String instance = "24838919";
		...
		SoftAssert softAssert = new SoftAssert();
		 ...
		 Boolean isActive =  HstryPge.verifyHistoryPageDisplayed();
		  ...
		  log.info("*************** Start Verify Null Borrower Name ***************");
		  	
		  // Verify Null Borrower Name
		  String lnName = LnRqstPge.checkName();
		  softAssert.assertEquals(lnName, "Null Borrower");
		  	
		  log.info("*************** End Verify Null Borrower Name ***************");
