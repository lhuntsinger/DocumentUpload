...
public class VerifyPhoneFieldCannotBeEditedForNullBorrowerTest extends TestBase {
...
// Test Case: Verify that Phone field cannot be edited for Null Borrower
// Scenario: User goes to Documents Upload URL, enters Null Borrower Application ID, verifies applicant Phone information,
//			 verifies that Phone field cannot be edited
// Expected Results: Phone field cannot be edited

	
	  @Test
	  public void verifyPhoneNotEditableForNullBorrower(ITestContext ctx) {
		  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
			
		String instance = "24837236";
		...
		SoftAssert softAssert = new SoftAssert();
		 ...
		 Boolean isActive =  HstryPge.verifyHistoryPageDisplayed();
		  ...
		  log.info("*************** Start Verify Null Borrower Phone ***************");
		  	
		  // Verify Null Borrower Phone
		  String lnPhone = LnRqstPge.checkPhone();
		  softAssert.assertEquals(lnPhone, "9123654780");
		  	
		  log.info("*************** End Verify Null Borrower Phone ***************");
