...
public class VerifyExpDateFieldCannotBeEditedForPrimaryJBTest extends TestBase {
...
// Test Case: Verify that Expiration Date field cannot be edited for Primary Joint Borrower
// Scenario: User goes to Documents Upload URL, enters Joint Borrower Application ID, verifies applicant Expiration Date information,
//			 verifies that Expiration Date field cannot be edited
// Expected Results: Expiration Date field cannot be edited

	
	  @Test
	  public void verifyExpDateNotEditableForPrimaryJointBorrower(ITestContext ctx) {
		  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
			
		String instance = "24863136";
		...
		SoftAssert softAssert = new SoftAssert();
		 ...
		 Boolean isActive =  HstryPge.verifyHistoryPageDisplayed();
		  ...
		  log.info("*************** Start Verify Primary Joint Borrower Expiration Date ***************");
		  	
		  // Verify Primary Joint Borrower Expiration Date
		  String lnExpDate = LnRqstPge.checkExpDate();
		  softAssert.assertEquals(lnExpDate, "May 15, 2020");
		  	
		  log.info("*************** End Verify Primary Joint Borrower Expiration Date ***************");
