...
public class VerifyBranchFieldCannotBeEditedForNullBorrowerTest extends TestBase {
...
// Test Case: Verify that Branch field cannot be edited for Null Borrower
// Scenario: User goes to Documents Upload URL, enters Null Borrower Application ID, verifies applicant Branch information,
//			 verifies that Branch field cannot be edited
// Expected Results: Branch field cannot be edited

	
	  @Test
	  public void verifyBranchNotEditableForNullBorrower(ITestContext ctx) {
		  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
			
		String instance = "24846450";
		...
		SoftAssert softAssert = new SoftAssert();
		 ...
		 Boolean isActive =  HstryPge.verifyHistoryPageDisplayed();
		  ...
		  log.info("*************** Start Verify Null Borrower Branch ***************");
		  	
		  // Verify Null Borrower Branch
		  String lnBranch = LnRqstPge.checkBranch();
		  softAssert.assertEquals(lnBranch, "0377 - Mauldin");
		  	
		  log.info("*************** End Verify Null Borrower Branch ***************");
