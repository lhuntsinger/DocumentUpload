...
public class VerifyExpDateFieldCannotBeEditedForIndividualBorrowerTest extends TestBase {
...
// Test Case: Verify that Expiration Date field cannot be edited for Individual Borrower
// Scenario: User goes to Documents Upload URL, enters Individual Borrower Application ID, verifies applicant Expiration Date information,
//			 verifies that Expiration Date field cannot be edited
// Expected Results: Expiration Date field cannot be edited

	
	  @Test
	  public void verifyExpDateNotEditableForIndividualBorrower(ITestContext ctx) {
		  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
			
		String instance = "24859432";
		...
		SoftAssert softAssert = new SoftAssert();
		 ...
		 Boolean isActive =  HstryPge.verifyHistoryPageDisplayed();
		  ...
		  log.info("*************** Start Verify Individual Borrower Expiration Date ***************");
		  	
		  // Verify Individual Borrower Expiration Date
		  String lnExpDate = LnRqstPge.checkExpDate();
		  softAssert.assertEquals(lnExpDate, "May 15, 2020");
		  	
		  log.info("*************** End Verify Individual Borrower Expiration Date ***************");
