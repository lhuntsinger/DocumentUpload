...
public class VerifyPhoneFieldCannotBeEditedForSecondaryJBTest extends TestBase {
...
// Test Case: Verify that Phone field cannot be edited for Secondary Joint Borrower
// Scenario: User goes to Documents Upload URL, enters Joint Borrower Application ID, selects Secondary Joint Borrower,
//		     verifies applicant Phone information, verifies that Phone field cannot be edited
// Expected Results: Phone field cannot be edited

	
	  @Test
	  public void verifyPhoneNotEditableForSecondaryJointBorrower(ITestContext ctx) {
		  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
			
		String instance = "24837017";
		...
		SoftAssert softAssert = new SoftAssert();
		 ...
		 Boolean isActive =  HstryPge.verifyHistoryPageDisplayed();
		  ...
		  // Select the Secondary Borrower name from drop down
		  ...
		  log.info("*************** End Select Secondary Joint Borrower Name ***************");
 		  ...
 		  log.info("*************** Start Verify Secondary Joint Borrower Phone ***************");
		  	
		  // Verify Secondary Joint Borrower Phone
		  String lnPhone = LnRqstPge.checkPhone();
		  softAssert.assertEquals(lnPhone, "");
		  	
		  log.info("*************** End Verify Secondary Joint Borrower Phone ***************");
