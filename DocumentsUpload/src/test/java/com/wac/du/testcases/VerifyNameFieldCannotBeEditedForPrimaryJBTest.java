...
public class VerifyNameFieldCannotBeEditedForPrimaryJBTest extends TestBase {
...
// Test Case: Verify that Name field cannot be edited for Primary Joint Borrower
// Scenario: User goes to Documents Upload URL, enters Joint Borrower Application ID, verifies applicant Name information,
//			 verifies that Name field cannot be edited
// Expected Results: Name field cannot be edited

	
	  @Test
	  public void verifyNameNotEditableForPrimaryJointBorrower(ITestContext ctx) {
		  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
			
		String instance = "24838916";
		...
		SoftAssert softAssert = new SoftAssert();
		 ...
		 Boolean isActive =  HstryPge.verifyHistoryPageDisplayed();
		  ...
		  log.info("*************** Start Verify Primary Joint Borrower Name ***************");
		  	
		  // Verify Primary Joint Borrower Name
		  String lnName = LnRqstPge.checkJBName();
		  softAssert.assertEquals(lnName, "Joint Borrower");
		  	
		  log.info("*************** End Verify Primary Joint Borrower Name ***************");
		  	
		  	
		  log.info("*************** Start Verify Primary Joint Borrower Name Field Not Editable ***************");
		  	
		  // Verify Primary Joint Borrower Name field cannot be edited
		  Boolean isDisabled = LnRqstPge.verifyPrimaryJBNameNotEditable();
		  softAssert.assertTrue(isDisabled);
		  	
		  log.info("*************** End Verify Primary Joint Borrower Name Field Not Editable ***************");
