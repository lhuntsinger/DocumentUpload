...
public class RequestProofOfIncDocByEmailTest extends TestBase {
...
// Test Case: Request Proof of Income Document by Email for Individual Borrower
// Scenario: User goes to Documents Upload URL, navigates to Loan Request page, enters Individual Borrower Application ID, 
// 			 verifies Notify via Email option is selected, selects Proof of Income from Required Documents 
// 			 and selects Send Request button
// Expected Results: Email should be received with Upload Document link for Proof of Income
	
	@Test
	public void requestPrfOfIncDocByEmail(ITestContext ctx) {
	  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
		
		String instance = "24695866";
		...
		LnRqstPge.clickRequest();
	  	...
	  	log.info("*************** Start Verify Required Doc Proof of Income visible ***************");
	  	
	  	// Verify Proof of Income is listed under Required Documents
	  	Boolean isDisplayed1 = LnRqstPge.checkReqDocIncome();
	  	softAssert.assertTrue(isDisplayed1);
 	
	  	log.info("*************** End Verify Required Doc Proof of Income visible ***************");
