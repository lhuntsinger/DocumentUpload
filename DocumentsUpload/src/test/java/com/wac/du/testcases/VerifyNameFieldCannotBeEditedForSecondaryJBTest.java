...
public class VerifyNameFieldCannotBeEditedForSecondaryJBTest extends TestBase {
...
// Test Case: Verify that Name field cannot be edited for Secondary Joint Borrower
// Scenario: User goes to Documents Upload URL, enters Joint Borrower Application ID, selects Secondary JB,
//		     verifies applicant Name information, verifies that Name field cannot be edited
// Expected Results: Name field cannot be edited

	
	  @Test
	  public void verifyEmailNotEditableForSecondaryJointBorrower(ITestContext ctx) {
		  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
			
		String instance = "24838920";
		...
		SoftAssert softAssert = new SoftAssert();
		 ...
		 Boolean isActive =  HstryPge.verifyHistoryPageDisplayed();
		  ...
		  log.info("*************** Start Verify Secondary Joint Borrower Name Field Not Editable ***************");
		  	
		  // Verify Email field cannot be edited
		  Boolean isDisabled = LnRqstPge.verifySecondaryJBNameNotEditable();
		  softAssert.assertTrue(isDisabled);
		  	
		  log.info("*************** End Verify Secondary Joint Borrower Name Field Not Editable ***************");
