...
public class VerifyExpDateFieldCannotBeEditedForSecondaryJBTest extends TestBase {
...
// Test Case: Verify that Expiration Date field cannot be edited for Secondary Joint Borrower
// Scenario: User goes to Documents Upload URL, enters Joint Borrower Application ID, selects Secondary Joint Borrower,
//		     verifies applicant Expiration Date information, verifies that Expiration Date field cannot be edited
// Expected Results: Expiration Date field cannot be edited

	
	  @Test
	  public void verifyExpDateNotEditableForSecondaryJointBorrower(ITestContext ctx) {
		  
		String dateAndTime = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());
			
		String instance = "24863199";
		...
		SoftAssert softAssert = new SoftAssert();
		 ...
		 Boolean isActive =  HstryPge.verifyHistoryPageDisplayed();
		  ...
		  log.info("*************** End Select Secondary Joint Borrower Name ***************");
 		  ...
 		  log.info("*************** Start Verify Secondary Joint Borrower Expiration Date ***************");
		  	
		  // Verify Secondary Joint Borrower Expiration Date
		  String lnExpDate = LnRqstPge.checkExpDate();
		  softAssert.assertEquals(lnExpDate, "May 15, 2020");
		  	
		  log.info("*************** End Verify Secondary Joint Borrower Expiration Date ***************");
